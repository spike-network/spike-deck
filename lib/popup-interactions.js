...
function installTaskHub(announce) {
  ...
  const render = () => {
    scheduled = false;
    const rows = [];
    const groupButtons = Array.from(document.querySelectorAll(".btn-test-group.testing"));
    if (groupButtons.length) {
      const row = document.createElement("div");
      row.className = "runtime-task-item";
      const label = document.createElement("span");
      label.textContent = `策略组测速 · ${groupButtons.length} 项运行中`;
      const cancel = document.createElement("button");
      cancel.type = "button";
      cancel.className = "btn-task-action";
      cancel.textContent = "全部取消";
      cancel.addEventListener("click", () => {
        for (const button of groupButtons) button.click();
        announce("已请求取消全部策略组测速任务");
      });
    ...
    if (moduleNotice?.textContent?.includes("正在")) {
      const row = document.createElement("div");
      row.className = "runtime-task-item";
