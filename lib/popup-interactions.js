...
function installSelectionFeedback(announce, shouldDisconnectAffectedConnections) {
  ...
  document.addEventListener("click", (event) => {
    ...
    message.textContent = shouldDisconnectAffectedConnections()
      ? `已请求切换到 ${nextName}；将断开受影响的旧连接`
      : `已请求切换到 ${nextName}；仅影响新连接`;
...
export function installPopupInteractions({
  getInstanceId,
  shouldDisconnectAffectedConnections = () => true,
}) {
  ...
  installSelectionFeedback(announce, shouldDisconnectAffectedConnections);
