...
const DISCONNECT_AFFECTED_CONNECTIONS_ON_SELECTION_KEY =
...
export class StorageManager {
  ...
  /** Whether route changes disconnect active connections still using the previous route. */
  static async shouldDisconnectAffectedConnectionsOnSelection() {
    const data = await chrome.storage.local.get([
      DISCONNECT_AFFECTED_CONNECTIONS_ON_SELECTION_KEY
    ]);
    return data[DISCONNECT_AFFECTED_CONNECTIONS_ON_SELECTION_KEY] !== false;
  }

  static async setDisconnectAffectedConnectionsOnSelection(enabled) {
    await chrome.storage.local.set({
      [DISCONNECT_AFFECTED_CONNECTIONS_ON_SELECTION_KEY]: Boolean(enabled)
    });
  }
