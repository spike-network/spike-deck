...
export class SpikeApiClient {
  ...
  static async request(instance, path, options = {}) {
    ...
    headers.set(CONTROL_API_VERSION_HEADER, String(CONTROL_API_VERSION));
    if (instance.secret) {
   ...
   * }>}
   */
  static async getStatus(instance, options = {}) {
    return await SpikeApiClient.request(instance, '/spike/status', options);
  }
   ...
   * @param {boolean} [retestActive]
   */
  static async selectGroupMember(instance, groupName, memberName, retestActive = true) {
    const encodedGroup = encodeURIComponent(groupName);
    return await SpikeApiClient.request(instance, `/spike/groups/${encodedGroup}/select`, {
      method: 'PUT',
      body: { member: memberName, retest_active: Boolean(retestActive) }
    });
  }
   ...
   */
  static async clearGroupSelection(instance, groupName, retestActive = true) {
    const encodedGroup = encodeURIComponent(groupName);
    return await SpikeApiClient.request(instance, `/spike/groups/${encodedGroup}/select`, {
      method: 'DELETE',
      body: { retest_active: Boolean(retestActive) }
    });
  }
  ...
  static async cancelGroupTestTask(instance, taskId) {
    return await SpikeApiClient.request(instance, `/spike/group-tests/${taskId}`, {
      method: 'DELETE'
    });
  }
   ...
   *     freshness?: 'fresh' | 'stale' | 'unmanaged' | 'unknown',
   *     refreshing?: boolean,
  ...
  static async getModuleUpdateTasks(instance) {
    return await SpikeApiClient.request(instance, '/spike/module-updates');
  }

  static async startModuleUpdateTask(instance, task, changes) {
    return await SpikeApiClient.request(instance, '/spike/module-updates', {
      method: 'POST',
      body: { id: task.id, task_namespace: task.taskNamespace, changes }
    });
  }
  ...
  static async evaluateScript(instance, body) {
    return await SpikeApiClient.request(instance, '/spike/scripts/evaluate', {
      method: 'POST',
      body,
      timeoutMs: 35000
    });
  ...
  static async evaluateCronScript(instance, name) {
    return await SpikeApiClient.request(instance, '/spike/scripts/cron/evaluate', {
      method: 'POST',
  ...
  static async getConnections(instance) {
    return await SpikeApiClient.request(instance, '/spike/connections');
  }

  static async getApiInfo(instance) {
    return validateControlApiInfo(await SpikeApiClient.request(instance, '/spike/api'));
  }

  static async getLogs(instance, limit = 200) {
    const requested = Number(limit);
    const bounded = Number.isFinite(requested)
      ? Math.max(1, Math.min(1000, Math.trunc(requested)))
      : 200;
    return await SpikeApiClient.request(instance, `/spike/logs?limit=${bounded}`);
  }

  static async getEvents(instance, limit = 200) {
    const requested = Number(limit);
    const bounded = Number.isFinite(requested)
      ? Math.max(1, Math.min(1000, Math.trunc(requested)))
      : 200;
    return await SpikeApiClient.request(instance, `/spike/events?limit=${bounded}`);
  }

  static async streamLogs(instance, { after = 0, namespace = '', signal, onOpen, onMessage } = {}) {
    const baseUrl = (instance.baseUrl || 'http://127.0.0.1:9090').replace(/\/+$/, '');
    const cursor = Number.isSafeInteger(Number(after)) && Number(after) > 0
      ? Math.trunc(Number(after))
      : 0;
    const namespaceQuery = namespace ? `&namespace=${encodeURIComponent(namespace)}` : '';
    const url = `${baseUrl}${SpikeApiClient.nativePath(`/spike/logs/stream?after=${cursor}${namespaceQuery}`)}`;
    const headers = new Headers({ Accept: 'text/event-stream' });
    headers.set(CONTROL_API_VERSION_HEADER, String(CONTROL_API_VERSION));
    if (instance.secret) headers.set('Authorization', `Bearer ${instance.secret}`);
    const response = await fetch(url, {
      method: 'GET',
      headers,
      signal,
    ...
    if (response.status === 401) throw new Error('Authentication failed (Secret incorrect)');
    if (!response.ok) {
      const error = new Error(`Spike API error: HTTP ${response.status}`);
      error.status = response.status;
      throw error;
    }
    const contentType = response.headers?.get?.('content-type') || '';
    if (!contentType.startsWith('text/event-stream')) {
      throw new Error('Spike API returned a non-SSE log stream');
    }
    onOpen?.();
    await consumeSseStream(response.body, (message) => onMessage?.(message));
  }

  static async previewReload(instance) {
    return await SpikeApiClient.request(instance, '/spike/reload/preview', {
      method: 'POST'
    });
  }

  static async killConnection(instance, id) {
    return await SpikeApiClient.request(instance, `/spike/connections/${encodeURIComponent(id)}`, {
      method: 'DELETE'
    });
  }

  static async explainRoute(instance, body) {
    return await SpikeApiClient.request(instance, '/spike/rules/explain', {
      method: 'POST',
      body
    });
  }

  static async queryDns(instance, body) {
    return await SpikeApiClient.request(instance, '/spike/dns/query', {
      method: 'POST',
      body
    });
  }

  static async flushDns(instance) {
    return await SpikeApiClient.request(instance, '/spike/dns/flush', {
      method: 'POST'
    });
  }
