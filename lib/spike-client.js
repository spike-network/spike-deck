...
export class SpikeApiClient {
  ...
  static async request(instance, path, options = {}) {
    ...
    const nativePath = path.startsWith('/spike') ? path : `/spike${path}`;
    const url = `${baseUrl}${nativePath}`;
  ...
  static async getOutbound(instance) {
    return await SpikeApiClient.request(instance, '/outbound');
  }
  ...
  static async getPolicies(instance) {
    return await SpikeApiClient.request(instance, '/policies');
  }
