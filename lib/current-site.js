...
function routePolicy(route) {
  if (route?.resolution) {
    return compactPolicy(route.resolution.policy_chain) ||
      compactPolicy(route.rule_policy) || "未返回线路";
  }
...
export function summarizeCurrentSite(target, route, connections) {
  ...
  return {
    ...
    resolutionError: String(route?.resolution?.error || "").trim() ||
      (route?.ok === false ? "线路解析失败" : null),
