...
await SpikeApiClient.getMetrics(instance);
await SpikeApiClient.getLogs(instance, 200);
await SpikeApiClient.getEvents(instance, 200);
...
assert.deepEqual(
  ...
  [
    ...
    'http://127.0.0.1:9090/spike/metrics.json',
    'http://127.0.0.1:9090/spike/logs?limit=200',
    'http://127.0.0.1:9090/spike/events?limit=200',
...
assert.equal(requests[19].options.method, 'DELETE');
...
assert.equal(requests[24].options.method, 'POST');
assert.equal(
