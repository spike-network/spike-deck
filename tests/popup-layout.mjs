...
assert.match(selectedRule, /white-space:\s*nowrap;/);
assert.match(selectedRule, /overflow:\s*hidden;/);
const selectedLeadingRule = css.match(/\.current-selected-leading\s*\{([^}]+)\}/)[1];
assert.match(selectedLeadingRule, /text-overflow:\s*ellipsis;/);
const selectedTrailingRule = css.match(/\.current-selected-trailing\s*\{([^}]+)\}/)[1];
assert.match(selectedTrailingRule, /flex-shrink:\s*0;/);
const groupNameRule = css.match(/\.group-name\s*\{([^}]+)\}/)[1];
assert.match(groupNameRule, /white-space:\s*nowrap;/);
assert.match(groupNameRule, /text-overflow:\s*ellipsis;/);
